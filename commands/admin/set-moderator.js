...
module.exports = class SetModeratorCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description:
        'Ajoute le role moderateur a tous les channels, et force les permissions.',
  ...
  async run(msg) {
    const hasRoleModerator = msg.guild.roles.cache.some(
      (role) => role.name === 'Modérateur'
    ); // Adaptez le nom si vous changez le nom du rôle modérateur
    if (!hasRoleModerator) {
      this.client.logger.log(
        'error',
        'Role \'Modérateur\' non trouvé sur ce serveur.'
      );
      return;
    }
    ...
    msg.guild.channels.cache.map((channel) => {
      this.client.logger.log(
        'info',
        `Role 'Modérateur' ajouté sur ${channel.name}.`
      );
    ...
    const embed = new Discord.MessageEmbed()
      .setDescription(
        `Mise à jour des permissions pour le role ${roleModerator.name}.`
      )
      .setColor('GREEN');
